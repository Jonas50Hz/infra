...
### 3.5 Fragmentation
- If frame ≤ 65535 B, a single frame (CONT_IDX=0) may be used, or fragmentation via CONT_IDX.
...
### 9.0 v2 vs v3 structural differences (summary)
...
- [ ] Timestamp reconstructed via SOC + FRACSEC/TIME_BASE (mind v2's 4-B FRACSEC vs v3's 1-B LEAP_BYTE + 3-B FRACSEC).
...
- [ ] Version detected via SYNC nibble; convert per §10 if mixing v2/v3.
