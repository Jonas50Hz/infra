...
> The messages defined in the main clause are **version 3**. Version 2 (2011) framing is in the "Version 2" section at the end.
...
| 10 | NUM_PMU | 2 | Number of PMUs in the data frame. |
...
### 9.1 v2 SYNC second-byte values (corrected)

For a genuine version-2 (C37.118.2-2011) frame, the version nibble is
`0010`.

| Frame | Type bits 6-4 | V2 second byte | Annex A V1 example |
|---|---|---:|---:|
| Data | `000` | `0x02` | `0x01` |
| Header | `001` | `0x12` | N/A |
| CFG-1 | `010` | `0x22` | `0x21` |
...
The first byte is always `0xAA`, so a V2 command begins `0xAA42`.
Table A.1 normatively assigns V2 nibble `0010`; the `...1` examples in Tables
A.5, A.8, and A.13 are explicitly labeled version 1 (IEEE Std
C37.118-2005) carryovers and must not be used for V2 fixtures.
...
- **SYNC 2nd byte:** V2 CFG-1 = `0x22`, V2 CFG-2 = `0x32`. Annex A's
  `0x21`/`0x31` examples are version-1 carryovers; see Section 9.1.
...
`SYNC(2)=0xAA42 | FRAMESIZE(2) | IDCODE(2) | SOC(4) | FRACSEC(4) | CMD(2) | EXTFRAME(0–65518) | CHK(2)`. Annex A's `0xAA41` illustration is version 1.
...
Per-field actions building an outgoing v2 config from incoming v3:
...
| PMU_NAME | STN | No change (renamed; note v2 STN is fixed 16 B ASCII). |
...
> Note: the mapping labeled here uses v3 field names as they appear in the standard's Table H.2 (voltage class dropped, PMUFLAG→SVC_CLASS+FNOM). PHSCALE shrinks 16→12 B/phasor by dropping only the voltage-class word.
...
- [ ] SYNC = `0xAA` + type byte (§9.1); version nibble `0001`/`0010`.
...
- [ ] SYNC = `0xAA` + correct type/version nibble.
...
- [ ] Timestamp reconstructed via SOC + FRACSEC/TIME_BASE.
...
- [ ] Version 2 devices detected via SYNC nibble and 4-B FRACSEC; converted per Annex H if mixing.
