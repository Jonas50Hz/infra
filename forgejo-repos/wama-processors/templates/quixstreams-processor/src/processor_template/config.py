"""Configuration loading for an application-owned Quixstreams processor."""
...
class ConfigurationError(ValueError):
    """Raised when an application processor configuration is invalid."""


@dataclass(frozen=True)
class ProcessorConfig:
    """Kafka settings owned by one application processor."""
...
def load_config(path: str | Path) -> ProcessorConfig:
    """Load one processor configuration before connecting to Kafka."""
    ...
    return ProcessorConfig(
        kafka_bootstrap_servers=_required_string(config, "kafka_bootstrap_servers"),
