...
from gateway_c37_118.codec import build_source_message, serialize_source_message
from gateway_c37_118.config import (
    C37_118V2SignalSelectorDefinition,
    Catalog,
    SignalDefinition,
    SourceDefinition,
)
from gateway_c37_118.reconciliation import ReconciliationError, reconcile_catalog
...
class ReconciliationTests(unittest.TestCase):
    ...
    def test_upserts_current_catalog_sources(self) -> None:
        source = self._source()
        existing = {
            source.source_id: self._payload(source, "old-revision", "wama-c37-118")
        }
        catalog = Catalog("wama-c37-118", "new-revision", (source,))
    ...
    def test_tombstones_removed_catalog_sources(self) -> None:
        ...
        plan = reconcile_catalog(
            Catalog("wama-c37-118", "new-revision", ()),
            existing,
            PUBLISHED_AT,
        )
    ...
    def test_rejects_mrid_mutation_for_an_existing_signal(self) -> None:
        ...
        with self.assertRaisesRegex(ReconciliationError, "immutable"):
            reconcile_catalog(
                Catalog("wama-c37-118", "new-revision", (changed,)),
                {
    ...
    def test_rejects_mrid_owned_by_another_source(self) -> None:
        ...
        with self.assertRaisesRegex(ReconciliationError, "already owned"):
            reconcile_catalog(
                Catalog("wama-c37-118", "new-revision", (proposed,)),
                {
