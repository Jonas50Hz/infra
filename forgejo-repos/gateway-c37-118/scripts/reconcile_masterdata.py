...
GATEWAY_COMPOSE_FILE = ".wama-forgejo-gateway-c37-118-gateways.json"
GATEWAY_STATE_FILE = ".wama-forgejo-gateway-c37-118-gateways.json.state"
...
class DeploymentError(RuntimeError):
    """Raised when a gateway-c37-118 deployment target is unsafe."""
...
def reconcile_masterdata(
    ...
    environment["WAMA_GATEWAY_C37_118_COMPOSE_PROJECT_NAME"] = project_name
...
def _validate_workspace(workspace: Path) -> None:
    if not (workspace / "compose.yaml").is_file() or not (workspace / "Dockerfile").is_file():
        raise DeploymentError("Forgejo checkout is not a gateway-c37-118 repository")
    if not (workspace / "catalog" / "sources").is_dir():
        raise DeploymentError("C37.118 gateway checkout has no source catalog")
    if (workspace / "docker-compose.yml").exists():
        raise DeploymentError("C37.118 gateway must not use an infrastructure checkout")


def _validate_deploy_root(workspace: Path, deploy_root: Path) -> Path:
    if not deploy_root.is_absolute():
        raise DeploymentError("WAMA_GATEWAY_C37_118_DEPLOY_ROOT must be an absolute path")
    if deploy_root.is_symlink():
        raise DeploymentError("WAMA_GATEWAY_C37_118_DEPLOY_ROOT must not be a symbolic link")
    deploy_root = deploy_root.resolve()
    if deploy_root == Path("/"):
        raise DeploymentError("WAMA_GATEWAY_C37_118_DEPLOY_ROOT must not be /")
    if deploy_root == workspace or deploy_root.is_relative_to(workspace):
        raise DeploymentError(
            "WAMA_GATEWAY_C37_118_DEPLOY_ROOT must not be inside the Forgejo workspace"
        )
    marker = deploy_root / DEPLOY_MARKER
    if marker.is_symlink() or not marker.is_file():
        raise DeploymentError(f"C37.118 gateway deploy root lacks {DEPLOY_MARKER}")
    return deploy_root
...
def _active_source_ids(deploy_root: Path) -> tuple[str, ...]:
    ...
    if source_directory.is_symlink() or not source_directory.is_dir():
        raise DeploymentError("C37.118 gateway deployment has no safe source catalog")
    source_ids: list[str] = []
    for source_path in sorted(source_directory.glob("*.yaml")):
        if source_path.is_symlink() or not source_path.is_file():
            raise DeploymentError(f"C37.118 gateway source is not a regular file: {source_path.name}")
        source_id = source_path.stem
        if not SOURCE_ID_PATTERN.fullmatch(source_id):
            raise DeploymentError(f"C37.118 gateway source ID is unsafe: {source_id!r}")
...
def _previous_gateway_services(deploy_root: Path) -> tuple[str, ...]:
    ...
    if state_path.is_symlink() or not state_path.is_file():
        raise DeploymentError("C37.118 gateway gateway state is unsafe")
    try:
        state = json.loads(state_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as error:
        raise DeploymentError("C37.118 gateway gateway state is invalid") from error
    ...
    if not isinstance(services, list) or not all(isinstance(service, str) for service in services):
        raise DeploymentError("C37.118 gateway gateway state is invalid")
    if len(services) != len(set(services)):
        raise DeploymentError("C37.118 gateway gateway state repeats a service")
...
def _write_gateway_compose(deploy_root: Path, source_ids: tuple[str, ...]) -> None:
    ...
    if compose_path.is_symlink():
        raise DeploymentError("C37.118 gateway generated Compose file is unsafe")
    ...
    for source_id in source_ids:
        ...
        services[service_name] = {
            ...
            "command": ["python", "-m", "gateway_c37_118.gateway_main"],
            "environment": {
                "KAFKA_BOOTSTRAP_SERVERS": "${KAFKA_BOOTSTRAP_SERVERS:-kafka:9092}",
                "WAMA_MASTERDATA_CATALOG_DIRECTORY": "/app/catalog/sources",
                "WAMA_MASTERDATA_CATALOG_ID": "${WAMA_MASTERDATA_CATALOG_ID:-wama-c37-118}",
...
def _rendered_compose_config(
    ...
    try:
        ...
        raise DeploymentError("Rendered gateway-c37-118 Compose config is not JSON") from error
...
def _require_expected_services(services: list[str], expected_services: tuple[str, ...]) -> None:
    if len(services) != len(expected_services) or set(services) != set(expected_services):
        ...
        raise DeploymentError(
            f"C37.118 gateway must contain only {expected_names}; found {names}"
        )


def _require_external_infra_network(rendered: Any, infra_network: str) -> None:
    if not isinstance(rendered, dict):
        raise DeploymentError("Rendered gateway-c37-118 Compose config must be an object")
    networks = rendered.get("networks")
    if not isinstance(networks, dict):
        raise DeploymentError("C37.118 gateway must declare the external wama-infra network")
    network = networks.get("wama-infra")
    if not isinstance(network, dict):
        raise DeploymentError("C37.118 gateway must declare the wama-infra network")
    if network.get("external") is not True or network.get("name") != infra_network:
        raise DeploymentError("C37.118 gateway must use only the existing external wama-infra network")
...
def _require_only_infra_network(rendered: Any, services: tuple[str, ...]) -> None:
    if not isinstance(rendered, dict) or not isinstance(rendered.get("services"), dict):
        raise DeploymentError("Rendered gateway-c37-118 Compose config has no services")
    rendered_services = rendered["services"]
    for service_name in services:
        service = rendered_services.get(service_name)
        if not isinstance(service, dict):
            raise DeploymentError(f"Rendered gateway-c37-118 service is missing: {service_name}")
        ...
        if isinstance(networks, list):
            ...
            raise DeploymentError(f"C37.118 gateway service has no network: {service_name}")
        if network_names != {"wama-infra"}:
            raise DeploymentError(
                f"C37.118 gateway service must use only wama-infra: {service_name}"
            )
...
def _validate_gateway_service_name(service: str) -> None:
    if not service.startswith(GATEWAY_SERVICE_PREFIX):
        raise DeploymentError(f"C37.118 gateway gateway service is unsafe: {service!r}")
...
def _verify_image_revision(image: str, commit: str) -> None:
    ...
    if revision != commit:
        raise DeploymentError(
            f"C37.118 gateway image revision {revision or '<missing>'} does not match {commit}"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=os.environ.get(
            "WAMA_GATEWAY_C37_118_COMPOSE_PROJECT_NAME",
            "wama-gateway-c37-118",
        ),
    )
    arguments = parser.parse_args()
    deploy_root_value = os.environ.get("WAMA_GATEWAY_C37_118_DEPLOY_ROOT")
    if not deploy_root_value:
        print(
            "Deployment failed: WAMA_GATEWAY_C37_118_DEPLOY_ROOT must be configured",
            file=sys.stderr,
        )
        return 1
    try:
