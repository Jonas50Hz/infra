"""Tests for the standalone apparent-power deployment guard."""
...
class DeploymentGuardTests(unittest.TestCase):
    """Keep apparent-power deployment isolated from other Compose projects."""
    ...
    def test_rejects_other_or_missing_compose_services(self) -> None:
        with self.assertRaisesRegex(DeploymentError, "processor-apparent-power"):
    ...
    def test_rejects_a_marked_root_outside_the_expected_base_child(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            expected = base / "processor-apparent-power"
            unexpected = root / "other" / "processor-apparent-power"
