...
`grafadruid-druid-datasource@1.7.0` plugin in `/opt/grafana/plugins`, outside the
state volume. The version is pinned because it supports the repository's Grafana
12.1.0 image.

The service provisions two datasources and read-only dashboard folders from the
...
No Grafana alert rules or Trino federation are provisioned in this PoC.