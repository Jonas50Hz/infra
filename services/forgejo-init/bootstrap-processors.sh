...
runner_connection() {
  ...
  cat <<EOF
    $connection_name:
      url: $runner_url
      uuid: $(cat "$runner_dir/$connection_name.uuid")
      token: $(cat "$runner_dir/$connection_name.secret")
EOF
}
...
runner_connections="$(
  ...
  runner_connection "$frequency_deploy_name"
  runner_connection "$apparent_ci_name"
  runner_connection "$apparent_deploy_name"
  runner_connection "$frequency_iec104_export_ci_name"
