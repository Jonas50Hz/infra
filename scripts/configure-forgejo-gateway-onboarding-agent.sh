...
identity_file=/data/forgejo-gateway-c37-118-onboarding-agent.identity
token_file=/data/forgejo-gateway-c37-118-onboarding-agent.token
...
usage() {
  printf '%s\n' "Usage: $0 [--checkout <gateway-c37-118-onboarding checkout>]" >&2
  exit 2
}
...
read_identity() {
  identity="$(docker compose --project-directory "$infrastructure_root" exec -T forgejo-runner cat "$identity_file")" \
    || fail "Unable to read the Forgejo gateway onboarding agent identity from forgejo-runner"
  owner=
  repository=
  username=
  while IFS='=' read -r key value; do
    case "$key" in
      ...
      *) fail "Forgejo gateway onboarding agent identity contains an unknown field" ;;
    esac
  done <<EOF
$identity
EOF
  validate_identifier "$owner" || fail "Forgejo gateway onboarding agent identity has an invalid owner"
  validate_identifier "$repository" || fail "Forgejo gateway onboarding agent identity has an invalid repository"
  validate_identifier "$username" || fail "Forgejo gateway onboarding agent identity has an invalid username"
}
...
parse_origin() {
  ...
  case "$origin" in
    http://* | https://*) ;;
    *) fail "The onboarding checkout origin must use HTTP or HTTPS" ;;
  esac
  case "$origin" in
    *"$expected_suffix") ;;
    *) fail "The onboarding checkout origin is not $owner/$repository" ;;
  esac
  base_url="${origin%"$expected_suffix"}"
  remainder="${base_url#*://}"
  case "$remainder" in
    '' | *@* | *\?* | *\#*) fail "The onboarding checkout origin must not contain credentials, a query, or a fragment" ;;
  esac
  protocol="${base_url%%://*}"
  host="${remainder%%/*}"
  ...
  path="${path#/}"
  case "$host" in
    '') fail "The onboarding checkout origin is missing a host" ;;
  esac
}
...
if [ "$checkout" = "$infrastructure_root" ] || [ "$checkout" = "$seed_checkout" ]; then
  fail "Refusing to configure the infrastructure checkout or tracked Forgejo seed"
fi
...
[ "$checkout" = "$worktree_root" ] || fail "The onboarding checkout must be the Git worktree root"
origin="$(git -C "$checkout" remote get-url origin)" \
  || fail "The onboarding checkout must have an origin remote"
parse_origin "$origin"

token="$(docker compose --project-directory "$infrastructure_root" exec -T forgejo-runner cat "$token_file")" \
  || fail "Unable to read the Forgejo gateway onboarding agent token from forgejo-runner"
[ -n "$token" ] || fail "Forgejo gateway onboarding agent token is empty"

config_root="${XDG_CONFIG_HOME:-$HOME/.config}"
credential_directory="$config_root/wama-forgejo"
credential_file="$credential_directory/gateway-c37-118-onboarding.credentials"
umask 077
mkdir -p "$credential_directory"
chmod 700 "$credential_directory"
temporary_credential_file="$(mktemp "$credential_directory/.gateway-c37-118-onboarding.credentials.XXXXXX")"
